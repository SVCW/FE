  ...
  useEffect(() => {
    if (search) {
      ...
      if (search.includes('title')) {
        ...
        http
          .get(
            `https://localhost:7083/api/Activity/get-activity-title?title=${title}`
          )
          .then((response) => {
            const updatedArrActivity = response?.data?.data.map((activity) => {
            ...
            console.log(updatedArrActivity);
      ...
      if (search.includes('userId')) {
        ...
        http
          .get(
            `https://localhost:7083/api/Activity/get-activity-by-userId?userId=${userId}`
          )
          .then((response) => {
            const updatedArrActivity = response?.data?.data.map((activity) => {
              const matchingComments = commentData?.filter(
        ...
        <div className="container">
          <div style={{ display: 'flex' }}>
            <h3 style={{ paddingRight: '10px' }}>
              <NavLink to="/home" style={{ TextDecoder: 'underline' }}>
                Trang chủ /{' '}
              </NavLink>
            </h3>
            {search?.includes('title') ? null : (
              <h3>
                <a href="#"> Danh sách các chiến dịch đã kết thúc</a>
              </h3>
            )}
          </div>
          {cmt?.map((item, index) => {
                        ...
                        <div className="more-post-optns">
                          ...
                          <ul>
                            ...
                            >
                              ...
                              Xem kết quả
                              <span>Xem chi tiết kết quả hoạt động</span>
                            </li>
                            {userID === item?.userId ? (
                              <li
                                onClick={() => {
                                  handleClick();
                                  formik8.setFieldValue(
                                    'activityId',
                                    item?.activityId
                                  );
                                }}
                              >
                                ...
                                <span>Cập nhật chi tiết kết quả hoạt động</span>
                              </li>
                            ) : (
                              <div></div>
                            )}
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        ...
                        <h4 style={{ color: '#1572b8' }}> {item?.title}</h4>
                      </div>
